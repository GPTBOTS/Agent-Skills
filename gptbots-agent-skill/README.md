...
│   ├── bot-config-fields.md               # service tips + variable/property .bot fields
...
└── scripts/
    ...
    ├── gptbots_config_validator/         # latest Bot field validation modules
...
The rules in `validate_gptbots_config.py` are ported from the real backend/frontend validation. Latest Bot field checks live in focused modules under `scripts/gptbots_config_validator/` and are called by the CLI; keep every module below 250 pure LOC. When the schema changes, re-sync against these sources and bump the `version` in `SKILL.md`:
...
- Backend `.../bean/entity/HumanConfig.java`, `BotCustomVariableEntity.java`, `UserPropertyEntity.java`, and `.../bean/enums/BotCustomVariableType.java`
